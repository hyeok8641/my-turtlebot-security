# 🔗 프로젝트 토픽 인터페이스 정의서 — 1인조

본 프로젝트의 데이터 흐름과 노드 간 통신을 제어하기 위해 합의된 최종 토픽 인터페이스 명세서입니다.

| 토픽명 | 메시지 타입 | 발행자 (Publisher) | 구독자 (Subscriber) | 주기 | 성격 | QoS | 설명 |
|---|---|---|---|---|---|---|---|
| `/cmd_vel` | geometry_msgs/msg/Twist | `navigation2` / `patrol_node` | `turtlebot3_node` (SBC) | 10Hz | 연속 | RELIABLE | 로봇의 실제 이동 속도 및 회전 속도 제어 명령 |
| `/detection_msg` | std_msgs/msg/String | `web_bridge` (스마트폰 AI) | `patrol_node` | 이벤트 발생 시 | 엣지 | RELIABLE | 스마트폰 AI가 감지한 상태값 (`safe` 또는 `fire`) 발행 |
| `/mode_status` | std_msgs/msg/String | `patrol_node` | `web_bridge` (대시보드) | 5Hz | 연속 | BEST_EFFORT | 대시보드 전광판에 표시할 로봇의 현재 상태 (`INIT` / `PATROL` / `EMERGENCY` / `MANUAL`) |
